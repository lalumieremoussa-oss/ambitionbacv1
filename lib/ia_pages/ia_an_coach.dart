// 📁 lib/ia_pages/ia_an_coach.dart
//
// 🔧 CORRECTIF : l'ancien code enveloppait `CoachAiService.sendAudio(...)`
// dans un DEUXIÈME timeout de 30s, alors que le service lui-même a un
// timeout de 45s. Résultat : cet écran coupait l'appel AVANT que le service
// ait la moindre chance de renvoyer sa propre erreur claire, et on tombait
// systématiquement dans le message générique "Le serveur met trop de temps
// à répondre" — même quand le vrai souci était ailleurs (clé API, quota,
// modèle Gemini indisponible, etc.).
//
// Le timeout est maintenant géré UNE SEULE FOIS, dans gemini_service.dart.
// Ici, on se contente de relayer fidèlement le message de CoachAiException
// (qui distingue déjà : pas de réseau / timeout / erreur serveur / erreur
// inattendue) au lieu de l'écraser par un message générique.
...
// ÉCRAN PRINCIPAL
// ============================================================================
class IaAnCoachScreen extends StatefulWidget {
  final String matiereTitre;
  const IaAnCoachScreen({super.key, required this.matiereTitre});

  @override
  State<IaAnCoachScreen> createState() => _IaAnCoachScreenState();
}

class _IaAnCoachScreenState extends State<IaAnCoachScreen> {
  CoachState _currentState = CoachState.idle;

  String _professor = 'Emma';
  ...
  Future<void> _initEngine() async {
    ...
    _historyBox = await Hive.openBox('english_coach_box_v2');

    final appDir = await getApplicationDocumentsDirectory();
    final audioDir = Directory('${appDir.path}/english_coach_audio_v2');
    ...
    try {
      _systemPromptTemplate =
          await rootBundle.loadString('assets/iac/ANG2.txt');
    } catch (_) {
      _systemPromptTemplate =
          "You are {professor}, a warm English oral coach for level {level}. Keep replies short and always end with a question.";
    }
  ...
  // renvoyé par CoachAiException au lieu de le remplacer.
  // ==========================================================================
  Future<void> _processAiInteraction({
    ...
    try {
      ...
      debugPrint('Erreur coach [${e.code}]: ${e.message}');
      ...
      // Erreur totalement inattendue (bug local, pas venant du service) :
      // on l'affiche telle quelle plutôt que de dire "connexion".
      debugPrint('Erreur coach non-CoachAiException: $e\n$st');
      setState(() => _currentState = CoachState.error);
      ...
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            onSelected: (val) {
              if (val == 'emma') _changeCoach('Emma');
              if (val == 'alex') _changeCoach('Alex');
              if (val == 'level') _showLevelDialog();
            },
            itemBuilder: (ctx) => [
              PopupMenuItem(
                  value: 'emma',
                  child: Text('Coach Emma ${_professor == "Emma" ? "✓" : ""}')),
              PopupMenuItem(
                  value: 'alex',
                  child: Text('Coach Alex ${_professor == "Alex" ? "✓" : ""}')),
