// 📁 lib/ia_pages/ia_hi_dissertation.dart
...
class IaHiDissertationScreen extends StatefulWidget {
  final String matiereTitre;

  const IaHiDissertationScreen({
    super.key,
    required this.matiereTitre,
  });

  @override
  State<IaHiDissertationScreen> createState() => _IaHiDissertationScreenState();
}

class _IaHiDissertationScreenState extends State<IaHiDissertationScreen>
  ...
  static const String _promptKey = 'HI1';
  ...
  static const String _historyType = 'dissertation_histoire';
  ...
  // ==========================================================
  // VÉRIFICATION ET GESTION DE L'UTILISATION (Supabase)
  // ==========================================================
  //
                  ...
                  Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        const Text(
                          'Prof IA vous a offert 10 analyses de sujets d’histoire. '
  ...
  Future<void> _submitSubject() async {
    ...
    if (subject.isEmpty) {
      _showElegantSnack(
        'Écris d’abord ton sujet d’histoire.',
    ...
    try {
      ...
      final validationError = _validateHistoryJson(decoded);
  ...
  // ==========================================================
  // VALIDATION DU JSON (spécifique à l'histoire)
  // ==========================================================

  String? _validateHistoryJson(Map<String, dynamic> json) {
    ...
    final analyse = json['analyse'];
    if (analyse is! Map) return 'Analyse invalide.';
    if (!analyse.containsKey('type_sujet') ||
        !analyse.containsKey('mots_cles') ||
        !analyse.containsKey('bornes_chronologiques') ||
        !analyse.containsKey('espace_geographique') ||
        !analyse.containsKey('consigne') ||
        !analyse.containsKey('probleme_historique') ||
        !analyse.containsKey('enjeux')) {
      return 'Analyse historique incomplète.';
    }

    final plan = json['plan'];
    if (plan is! Map) return 'Plan invalide.';
    final parties = plan['parties'];
    if (parties is! List || parties.isEmpty) {
      return 'Au moins une partie est requise.';
    }

    for (final part in parties) {
      if (part is! Map) return 'Partie invalide.';
      final args = part['arguments'];
      if (args is! List || args.isEmpty) {
        return 'Chaque partie doit avoir au moins un argument.';
      }
      for (final arg in args) {
        if (arg is! Map) return 'Argument invalide.';
        if (!arg.containsKey('idee') ||
            !arg.containsKey('explication') ||
            !arg.containsKey('fait_historique') ||
            !arg.containsKey('analyse_du_fait')) {
          return 'Chaque argument doit avoir une idée, une explication, un fait historique et une analyse.';
        }
        final fait = arg['fait_historique'];
        if (fait is! Map || !fait.containsKey('evenement')) {
          return 'Fait historique invalide.';
        }
      }
    }

    final intro = json['introduction'];
    if (intro is! Map ||
        !intro.containsKey('generalite') ||
        !intro.containsKey('problematique')) {
      return 'Introduction incomplète.';
    }

    final controle = json['controle'];
    if (controle is! Map) return 'Contrôle invalide.';
    if (controle['plan_adapte_au_sujet'] != true) {
      return 'Le plan doit être adapté au sujet.';
    }
    if (controle['consigne_respectee'] != true) {
      return 'La consigne doit être respectée.';
    }
    if (controle['chronologie_respectee'] != true) {
      return 'La chronologie doit être respectée.';
    }
    if (controle['faits_precis'] != true) {
      return 'Des faits précis sont exigés.';
    }
    if (controle['introduction_redigee'] != false ||
        controle['conclusion_redigee'] != false ||
        controle['dissertation_complete'] != false) {
      return 'La réponse ne doit pas contenir de dissertation entièrement rédigée.';
    }
            ...
            Container(
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Prof IA est spécialisé en dissertation d’histoire. Envoie un sujet d’histoire pour obtenir un plan détaillé.',
                      style: TextStyle(
                        color: texteSecondaire,
                        fontSize: 11.5,
                        height: 1.4,
          ...
          children: [
            ...
            Column(
              ...
              children: [
                ...
                Text(
                  'Prof IA analyse ton sujet historique et construit un plan adapté avec des faits précis.',
                  style: TextStyle(
                ...
                Row(
                  children: [
                    _heroBadge(Icons.account_tree_rounded, 'ANALYSE'),
                    const SizedBox(width: 8),
                    _heroBadge(Icons.menu_book_rounded, 'FAITS'),
                    const SizedBox(width: 8),
                    _heroBadge(Icons.compare_arrows_rounded, 'PLAN'),
                  ],
                ),
              ],
            ),
            ...
            const Expanded(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 4),
                  Text(
                    'Analyse du sujet • construction du plan • faits historiques',
                    style: TextStyle(
                      fontSize: 10.5,
                      height: 1.35,
                      color: texteSecondaire,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _errorMessage ??
                  'Désolé, je suis là pour t’aider à travailler ta dissertation d’histoire.',
              style: const TextStyle(
                color: texte,
                fontSize: 14,
                height: 1.5,
            ...
            Container(
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Envoie un sujet de dissertation d’histoire pour commencer.',
                      style: TextStyle(
                        color: texteSecondaire,
                        fontSize: 11.5,
                        height: 1.4,
  ...
  // ==========================================================
  // RESULTAT PRINCIPAL (adapté à l'histoire)
  // ==========================================================
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 15),
          _buildAnalyse(result),
          const SizedBox(height: 15),
          _buildIntroduction(result),
          const SizedBox(height: 15),
          _buildPlan(result),
          const SizedBox(height: 15),
  ...
  Widget _buildAnalyse(Map<String, dynamic> result) {
    final analyse = Map<String, dynamic>.from(
      result['analyse'] ?? {},
    );

    final motsCles = (analyse['mots_cles'] as List?) ?? [];
    final bornes = Map<String, dynamic>.from(
      analyse['bornes_chronologiques'] ?? {},
    );

    return _sectionContainer(
      accent: vert,
      icon: Icons.search_rounded,
      label: 'ANALYSE DU SUJET',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _miniLabel('Type de sujet'),
          const SizedBox(height: 5),
          _textBlock(analyse['type_sujet']),
          const SizedBox(height: 16),
          ...
          ...motsCles.map(
            (item) {
              ...
              return _historyDefinitionTile(
                d['terme']?.toString() ?? '',
                d['definition_historique']?.toString() ?? '',
                importance: d['importance']?.toString() ?? '',
              );
            },
          ),
          const SizedBox(height: 16),
          _miniLabel('Bornes chronologiques'),
          const SizedBox(height: 7),
          _textBlock('Début : ${bornes['debut'] ?? ''}'),
          _textBlock('Fin : ${bornes['fin'] ?? ''}'),
          if (bornes['justification'] != null &&
              bornes['justification'].toString().isNotEmpty) ...[
            const SizedBox(height: 5),
            _textBlock('Justification : ${bornes['justification']}'),
          ],
          const SizedBox(height: 16),
          _miniLabel('Espace géographique'),
          const SizedBox(height: 7),
          _textBlock(analyse['espace_geographique']),
          const SizedBox(height: 16),
          _miniLabel('Consigne'),
          const SizedBox(height: 7),
          _textBlock(analyse['consigne']),
          const SizedBox(height: 16),
          _miniLabel('Problème historique'),
          const SizedBox(height: 7),
          Container(
            ...
            child: Text(
              analyse['probleme_historique']?.toString() ?? '',
              style: const TextStyle(
                color: texte,
                fontSize: 14,
                fontWeight: FontWeight.w700,
  ...
  Widget _buildIntroduction(Map<String, dynamic> result) {
    ...
    return _sectionContainer(
      ...
      label: 'INTRODUCTION — INDICATIONS',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _miniLabel('Généralité'),
          const SizedBox(height: 5),
          _textBlock(intro['generalite']),
          const SizedBox(height: 12),
          _miniLabel('Définitions'),
          const SizedBox(height: 5),
          _textBlock(intro['definitions']),
          const SizedBox(height: 12),
          _miniLabel('Bornes chronologiques'),
          const SizedBox(height: 5),
          _textBlock(intro['bornes']),
          const SizedBox(height: 12),
          _miniLabel('Cadre géographique'),
          const SizedBox(height: 5),
          _textBlock(intro['cadre_geographique']),
          const SizedBox(height: 12),
          _miniLabel('Problématique'),
          const SizedBox(height: 5),
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
          ...
          const SizedBox(height: 12),
          _miniLabel('Annonce du plan'),
          const SizedBox(height: 5),
          _textBlock(intro['annonce_plan']),
        ],
      ),
    );
  }
  ...
  // ==========================================================
  // PLAN (spécifique à l'histoire)
  // ==========================================================

  Widget _buildPlan(Map<String, dynamic> result) {
    final plan = Map<String, dynamic>.from(
      result['plan'] ?? {},
    );

    final typePlan = plan['type']?.toString() ?? '';
    final parties = (plan['parties'] as List?) ?? [];
      ...
      child: Column(
        ...
        children: [
          ...
          ...parties.asMap().entries.map((entry) {
            ...
            return _buildPartPlan(
              numero: index + 1,
              titre: part['titre']?.toString() ?? '',
              ideePrincipale: part['idee_principale']?.toString() ?? '',
              arguments: (part['arguments'] as List?) ?? [],
              transition: part['transition']?.toString() ?? '',
            );
          }).toList(),
        ],
      ),
  ...
  Widget _buildPartPlan({
    required int numero,
    required String titre,
    required String ideePrincipale,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _textBlock(ideePrincipale),
          const SizedBox(height: 12),
          _miniLabel('Arguments'),
          const SizedBox(height: 6),
          ...arguments.map((arg) {
            final a = Map<String, dynamic>.from(arg);
            return _historyArgumentCard(
              numero: a['numero']?.toString() ?? '',
              idee: a['idee']?.toString() ?? '',
              explication: a['explication']?.toString() ?? '',
              faitHistorique: a['fait_historique'] as Map? ?? {},
              analyseDuFait: a['analyse_du_fait']?.toString() ?? '',
            );
          }).toList(),
          if (transition.isNotEmpty) ...[
            const SizedBox(height: 12),
            _miniLabel('Transition'),
            const SizedBox(height: 4),
            _textBlock(transition),
          ],
        ],
      ),
    );
  ...
  Widget _historyArgumentCard({
    required String numero,
    required String idee,
    required String explication,
    required Map faitHistorique,
    required String analyseDuFait,
  }) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 3),
          _textBlock(explication),
          const SizedBox(height: 8),
          _miniLabel('Fait historique'),
          const SizedBox(height: 3),
          Container(
            padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _textBlock(faitHistorique['evenement'] ?? ''),
                if (faitHistorique['date'] != null &&
                    faitHistorique['date'].toString().isNotEmpty)
                  _textBlock('Date : ${faitHistorique['date']}'),
                if (faitHistorique['lieu'] != null &&
                    faitHistorique['lieu'].toString().isNotEmpty)
                  _textBlock('Lieu : ${faitHistorique['lieu']}'),
                if (faitHistorique['acteurs'] != null &&
                    faitHistorique['acteurs'].toString().isNotEmpty)
                  _textBlock('Acteurs : ${faitHistorique['acteurs']}'),
              ],
            ),
          ),
          const SizedBox(height: 8),
          _miniLabel('Analyse du fait'),
          const SizedBox(height: 3),
          _textBlock(analyseDuFait),
        ],
      ),
    );
  }
  ...
  Widget _buildConclusion(Map<String, dynamic> result) {
      ...
      child: Column(
        children: [
          _conclusionItem(
            ...
            title: 'Résumé',
            text: conclusion['resume']?.toString() ?? '',
            color: vert,
          ),
          ...
          _conclusionItem(
            ...
            text: conclusion['reponse_problematique']?.toString() ?? '',
            color: orange,
          ),
          const SizedBox(height: 11),
          _conclusionItem(
            ...
            text: conclusion['bilan_historique']?.toString() ?? '',
            color: Colors.blue,
          ),
          const SizedBox(height: 11),
          _conclusionItem(
          ...
          Container(
            ...
            child: const Row(
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    'À toi maintenant de transformer ce plan en dissertation. Prof IA te donne la direction, mais c’est toi qui construis ton analyse historique.',
                    style: TextStyle(
                      color: texteSecondaire,
                      fontSize: 11.5,
                      height: 1.45,
