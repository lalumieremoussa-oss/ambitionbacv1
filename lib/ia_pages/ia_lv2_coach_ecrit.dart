// 📁 lib/ia_pages/ia_lv2_conversation.dart
//
// Écran de CONVERSATION RÉELLE avec Prof IA, pour l'allemand (DE) ou
// l'espagnol (ES), paramétré via lv2Code. Même logique que
// ia_an_conversation.dart : fil de discussion continu, entrée texte OU
// vocale, réponse IA en JSON segmenté lue automatiquement en audio,
// avec bouton de ré-écoute.
//
// RÈGLE AUDIO (v2) : seule la langue cible (DE ou ES) est vocalisée.
...
class IaLv2ConversationPage extends StatefulWidget {
  final String? matiereTitre;
  final String lv2Code; // "DE" ou "ES"

  const IaLv2ConversationPage({
    super.key,
    this.matiereTitre,
  ...
  @override
  State<IaLv2ConversationPage> createState() => _IaLv2ConversationPageState();
}
...
class _IaLv2ConversationPageState extends State<IaLv2ConversationPage>
  ...
  static const String _threadKeyPrefix = 'lv2_conversation_thread_';
  static const String _voiceKeyPrefix = 'prof_ia_lv2_conv_voice_';
  static const String _defaultVoice =
      'Alex'; // stocke le GENRE (Alex=H, Emma=F)
  ...
  // =========================================================================
  // GETTERS DÉPENDANTS DE LA LANGUE
  // =========================================================================

  String get _langCode => widget.lv2Code.toUpperCase(); // "DE" ou "ES"
  String get _targetLangue => _langCode == 'DE' ? 'de' : 'es'; // pour le JSON
  String get _langName => _langCode == 'DE' ? 'allemand' : 'espagnol';
  String get _promptKey => '${_langCode}2'; // DE2 ou ES2
  String get _voiceKey => '$_voiceKeyPrefix$_langCode';

  List<String> get _starterTopics => _langCode == 'DE'
      ? [
          "Lass uns über Zahlen sprechen",
          "Explique-moi une leçon",
          "Lass uns über meinen Tag sprechen",
  ...
  String _voice = _defaultVoice; // 'Alex' ou 'Emma' -> genre générique
  ...
  // =========================================================================
  // VOIX (genre générique Alex/Emma, résolu en nom local à la lecture)
  // =========================================================================
  ...
  /// Résout le nom de voix propre à la langue cible (Anna/Mark, Lola/Diego)
  /// à partir du genre générique stocké (Alex = masculin, Emma = féminin).
  String _resolveTargetVoiceName() {
    final isFemale = _voice.toLowerCase() == 'emma';
    if (_langCode == 'DE') return isFemale ? 'Anna' : 'Mark';
    return isFemale ? 'Lola' : 'Diego'; // ES
  }
  ...
  // =========================================================================
  // HISTORIQUE (FIL DE DISCUSSION CONTINU, PAR LANGUE)
  // =========================================================================
  ...
  String _threadStorageKey(String? matricule) {
    final base = '$_threadKeyPrefix$_langCode';
    return matricule == null || matricule.isEmpty ? base : '${base}_$matricule';
  }
  ...
  Future<int> _getRemainingCalls() async {
    ...
    try {
      ...
      debugPrint('Erreur quota conversation LV2 : $e');
      return -1;
    }
  }
  ...
  Future<void> _showLimitDialog() async {
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'Tu as utilisé tes 10 échanges gratuits avec Prof IA en $_langName. '
            'Passe à Premium pour continuer sans limite.'),
        actions: [
          TextButton(
  ...
  String _buildPromptWithContext(String currentMessage) {
    ...
    while (i >= 1 && pairsFound < 2) {
      ...
      if (iaMsg.role == 'ia' && userMsg.role == 'user') {
        ...
        pairs.insert(0, 'Élève : ${userMsg.text ?? ''}\nProf IA : $iaText');
        pairsFound++;
        i -= 2;
      } else {
        i -= 1;
  ...
  void _validateConversation(Map<String, dynamic> data) {
    ...
    for (final seg in segments) {
      ...
      if ((langue != 'fr' && langue != _targetLangue) || texte.isEmpty) {
  ...
  // il n'est jamais lu automatiquement. Seuls les segments dans la langue
  // cible (DE/ES) et la question de relance déclenchent une lecture audio.

  Future<void> _speakSegmentText(String texte, String langue) async {
    if (texte.trim().isEmpty) return;
    if (langue != _targetLangue) return; // fr = texte only, jamais vocalisé

    final voiceName = _resolveTargetVoiceName();
    if (_langCode == 'DE') {
      await ServicesVocalDe.speak(text: texte, voice: voiceName);
    } else {
      await ServicesVocalEs.speak(text: texte, voice: voiceName);
    }
  }
    ...
    try {
      ...
      if (segments is List) {
        for (final seg in segments) {
          ...
          await _speakSegmentText(texte, langue);
        }
      }
      final question = data['question_suivante'];
      if (question is Map) {
        final texte = question['texte']?.toString().trim() ?? '';
        final langue = question['langue']?.toString() ?? _targetLangue;
        await _speakSegmentText(texte, langue);
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Text(
                      'Prof IA • ${_langName[0].toUpperCase()}${_langName.substring(1)}',
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Discute avec Prof IA en $_langName',
            ...
            const Text(
              ...
              'et avance avec toi, en français et dans la langue choisie.',
              textAlign: TextAlign.center,
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: _isListening
                        ? 'Je t’écoute...'
                        : 'Écris en français ou en $_langName...',
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                _langCode == 'DE' ? 'Anna / Mark' : 'Lola / Diego',
                style: const TextStyle(color: _secondary, fontSize: 11.5),
              ),
              const SizedBox(height: 14),
              _voiceOption('Alex', Icons.male_rounded,
                  _langCode == 'DE' ? 'Mark' : 'Diego'),
              const SizedBox(height: 8),
              _voiceOption('Emma', Icons.female_rounded,
                  _langCode == 'DE' ? 'Anna' : 'Lola'),
            ],
          ),
        );
      },
  ...
  Widget _voiceOption(String genericName, IconData icon, String localName) {
    final selected = _voice == genericName;
    return Material(
