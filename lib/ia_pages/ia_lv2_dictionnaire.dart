// 📁 lib/ia_pages/ia_lv2_dictionnaire.dart
...
class IaLv2DictionnaireScreen extends StatefulWidget {
  final String? matiereTitre;
  final String lv2Code; // "DE" ou "ES"

  const IaLv2DictionnaireScreen({
    super.key,
    this.matiereTitre,
  ...
  @override
  State<IaLv2DictionnaireScreen> createState() =>
      _IaLv2DictionnaireScreenState();
}

class _IaLv2DictionnaireScreenState extends State<IaLv2DictionnaireScreen>
  ...
  static const String _voiceKeyPrefix = 'prof_ia_lv2_voice_';
  ...
  String get _historyType => 'dictionnaire_lv2_${widget.lv2Code}';
  String get _voiceKey => '$_voiceKeyPrefix${widget.lv2Code}';
  String get _promptKey => '${widget.lv2Code}1'; // DE1 ou ES1
                  ...
                  Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        Text(
                          'Prof IA vous a offert 10 recherches de dictionnaire en ${widget.lv2Code == 'DE' ? 'allemand' : 'espagnol'}. '
  ...
  Future<void> _searchWord() async {
    ...
    if (word.isEmpty) {
      _showMessage(
        'Écris un mot en ${widget.lv2Code == 'DE' ? 'allemand' : 'espagnol'} ou en français.',
  ...
  void _validateDictionary(Map<String, dynamic> data) {
    ...
    final nom = data['nom'];
    if (nom is! Map) {
      throw const FormatException('Les informations du nom sont invalides.');
    }
  ...
  Future<void> _speak(String text, {String? key}) async {
    ...
    try {
      final bool isFemale = _voice.toLowerCase() == 'emma';

      if (widget.lv2Code == 'DE') {
        final String voiceName = isFemale ? 'Anna' : 'Mark';
        await ServicesVocalDe.speak(
          text: cleanText,
          voice: voiceName,
        );
      } else if (widget.lv2Code == 'ES') {
        final String voiceName = isFemale ? 'Lola' : 'Diego';
        await ServicesVocalEs.speak(
          text: cleanText,
          voice: voiceName,
        );
      } else {
  ...
  bool _isNoun() {
    ...
    if (data == null) return false;
    final nom = data['nom'];
    if (nom is! Map) return false;
    return nom['est_nom'] == true;
  }

  String _nounInfo() {
    final data = _dictionary;
    if (data == null) return '';
    final nom = data['nom'] as Map<String, dynamic>?;
    if (nom == null) return '';

    if (widget.lv2Code == 'DE') {
      final article = nom['article']?.toString() ?? '';
      final pluriel = nom['pluriel']?.toString() ?? '';
      if (article.isEmpty && pluriel.isEmpty) return '';
      return '$article • Pluriel : $pluriel';
    } else {
      final genre = nom['genre']?.toString() ?? '';
      final pluriel = nom['pluriel']?.toString() ?? '';
      if (genre.isEmpty && pluriel.isEmpty) return '';
      return '$genre • Pluriel : $pluriel';
    }
  }
  ...
  Widget _buildTopBar() {
    final langName = widget.lv2Code == 'DE' ? 'ALLEMAND' : 'ESPAGNOL';
          ...
          Column(
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  _heroMiniChip(
                      Icons.language_rounded, 'FR ↔ ${widget.lv2Code}'),
  ...
  Widget _buildSearchBox() {
    final hint = widget.lv2Code == 'DE'
        ? 'Écris un mot : Haus, Schule, laufen...'
        : 'Écris un mot : casa, escuela, correr...';
  ...
  Widget _buildEmptyState() {
    final suggestions = widget.lv2Code == 'DE'
        ? ['Haus', 'Schule', 'laufen', 'apprendre']
        : ['casa', 'escuela', 'correr', 'apprendre'];
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 7),
          const Text(
            'Tape un mot en français ou dans la langue '
          ...
          Wrap(
            ...
            spacing: 8,
            runSpacing: 8,
            children: suggestions.map((word) => _suggestionChip(word)).toList(),
          ),
        ],
      ),
    );
        ...
        children: [
          ...
          if (nounInfo.isNotEmpty) ...[
            ...
            Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.tag_rounded, color: _blue600, size: 16),
          ...
          Row(
            children: [
              ...
              if (_isNoun()) ...[
                const SizedBox(width: 7),
                _infoPill(Icons.text_fields_rounded, 'Nom'),
              ],
            ],
          ),
        ],
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _voiceChoice(
                  name: 'Alex',
                  subtitle: 'Voix masculine',
                  icon: Icons.male_rounded),
              const SizedBox(height: 9),
              _voiceChoice(
                  name: 'Emma',
                  subtitle: 'Voix féminine',
